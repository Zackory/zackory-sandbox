...
class HeatMap(object):
    ...
    def addPoints(self, points, radius):
...
def test1():
    ...
    def run():
        hm.addPoints(points, 10.5)
        hm.transformColor(1.0)
        image = hm.getImage()
        open("test1.png", "wb").write(image.read())

    def runtimes(ntimes):
        start = time.time()
        for i in xrange(ntimes):
            hm.clearFramebuffers()
            run()
...
def test2():
    ...
    hm.addPoints(points, 300)
    hm.transformColor(1.0)
    image = hm.getImage()
    open("test2.png", "wb").write(image.read())
...
def test3():
    ...
    print points[:10]
    hm.addPoints(points, 30)
    hm.transformColor(1.0)
    image = hm.getImage()
    open("test3.png", "wb").write(image.read())
